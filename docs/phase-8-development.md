...
Read [deployment](deployment.md), [security](security.md), [operations](operations-runbook.md), [backup/restore](backup-restore.md) and the [release checklist](release-checklist.md) before considering hosted activation. The subsequent specification audit adds the guarded crawler and explicit runtime/privilege profiles; local and legacy hosted gates remain preserved. Credentials alone do not enable production: authorized schema/role provisioning and live acceptance are still required. Live Reddit must remain disabled without explicit approved commercial access. Extension insertion never submits a Reddit comment.
