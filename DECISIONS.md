...
## ADR-042: Initiate Google sign-in without a cross-origin form redirect

Chromium blocks the native Google form's cross-origin 303 under the application's `form-action 'self'` policy. Keep that policy: request the authorization destination through a same-origin JSON POST, persist Supabase's PKCE cookies, validate the exact configured HTTPS Supabase origin/path/provider, then perform browser navigation. Show pending and retryable errors on the login page. Do not disable CSP, origin validation, Redis limits, server-verified identity or the local Google restriction. A configuration-failed callback uses a relative login redirect rather than a hardcoded localhost origin.

Read-only checks found Vercel SSO protection on the supplied deployment URL and Google disabled in the owner's previously supplied Supabase project's public settings. Provider enablement, Google client secrets, canonical URL/allowlist and SMTP delivery are owner-configured external prerequisites, not inferred from a successful web build. Local browser fixtures establish transport behavior without logging into Google or sending external email; hosted acceptance remains separate.
