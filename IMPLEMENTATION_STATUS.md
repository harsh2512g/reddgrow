...
- Active branch: main; current committed configuration `5ee6fcd` (`fix`), after source release `95f78b0`. The owner reported a further Vercel pnpm version mismatch: installation used 12.4.1 but the explicit Corepack build used 12.2.1. The build now calls pnpm directly and logs its version; executing that exact command from `apps/web` locally selected 12.4.1 and passed all 16 web/dependency builds. Lint, formatting and 21 install/isolation tests passed. The hosted rerun remains unverified; no cloud login, migration, deployment, commit or push was performed.
...
- Local app address: `http://127.0.0.1:3000`; development web and worker are running again. After the Vercel install fix, restoration checks returned HTTP 200 for the homepage, web readiness and worker readiness. The personal hosted project is unchanged.
