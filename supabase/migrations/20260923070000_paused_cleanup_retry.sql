-- A pause never prevents retrying physical cleanup of an already-deleted knowledge source.
-- Organization -> brand -> source -> job locking and the existing tombstone/generation checks remain.
...
begin
  ...
  if v_org is not null then
    perform 1 from public.organizations where id=v_org and deleted_at is null and (
      status='active' or (status='suspended' and p_family='knowledge' and exists(
        select 1 from public.knowledge_jobs cleanup join public.knowledge_sources source on source.id=cleanup.source_id
        where cleanup.id=p_job_id and cleanup.kind='delete' and source.deleted_at is not null
      ))) for update;
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(c) order by created_at desc,id desc),'[]'::jsonb) into v_items from (
    select j.*,j.created_at>now()-interval '90 days' and j.status='failed' and not exists(select 1 from private.platform_job_retries r where r.family=j.family and r.job_id=j.id)
      and (j.organization_id is null or exists(select 1 from public.organizations o where o.id=j.organization_id and ((o.status='active' and o.deleted_at is null) or (j.family='privacy' and j.kind='delete' and o.status='deleted') or (j.family='knowledge' and j.kind='delete' and o.status='suspended' and o.deleted_at is null))))
