-- Both deduplication identities must resolve to the same event and payload.
-- An older matching external ID must never hide a conflicting idempotency key.
...
begin
  ...
  if exists(select 1 from public.conversion_events e where e.brand_id=c.brand_id and ((e.external_id=v_external and e.event_type=p_event->>'event') or e.idempotency_key=v_idempotency) and e.payload_hash<>v_hash)
    or (select count(*) from public.conversion_events e where e.brand_id=c.brand_id and ((e.external_id=v_external and e.event_type=p_event->>'event') or e.idempotency_key=v_idempotency))>1 then raise exception 'CONVERSION_IDEMPOTENCY_CONFLICT'; end if;
  ...
  if v_date<c.occurred_at or v_date>c.occurred_at+make_interval(days=>v_days) or (v_source='browser' and now()>c.occurred_at+make_interval(days=>v_days)+interval '5 minutes') then raise exception 'CONVERSION_OUTSIDE_WINDOW'; end if;
