-- Match the shared URL validator before recording a click, including legacy numeric
-- host spellings and invalid DNS labels that an authenticated direct RPC might supply.
...
begin
  ...
  if v_tld ~ '^[0-9]+$' or v_tld in ('localhost','local','internal','lan','home','onion','invalid') then return null; end if;
  return v_host;
end $$;

-- A trusted server may deliver a delayed event whose actual occurrence was inside
-- the attribution window. Browser proofs additionally expire at the receive deadline.
